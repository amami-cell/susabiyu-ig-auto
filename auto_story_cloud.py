...
def refresh_token(sheets,token):
    ...
    try:
        r=req.get(f"{B}/refresh_access_token",params={"grant_type":"ig_refresh_token","access_token":token}).json()
        if "access_token" in r:
            ...
            try:
                ...
                try:
                    sheets.spreadsheets().values().get(spreadsheetId=SHEET_ID,range="Config!A1").execute()
                except:
                    sheets.spreadsheets().batchUpdate(spreadsheetId=SHEET_ID,body={"requests":[{"addSheet":{"properties":{"title":"Config"}}}]}).execute()
                sheets.spreadsheets().values().update(spreadsheetId=SHEET_ID,range="Config!A1:C2",valueInputOption="RAW",body={"values":[["token","updated","expires_days"],[new_token,now,str(days)]]}).execute()
