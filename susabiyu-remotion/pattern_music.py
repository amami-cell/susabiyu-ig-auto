...
HOOK = {
    ...
    # No.35 / No.37 は採用が決まったので、曲・OP・文言の3つとも固定する。
    # 曲は上の MUSIC、OPは各コンポーネントの Shell v=（コード側で固定済み。
    # No.35 は v=4／No.37 は v=7）、文言はここ。
    # これで焼き直しても毎回まったく同じものが出る。
    "yoshokuritmo": "旨いを、遠慮なく。",
    "yoshokuonda": "日常に、ひと皿の贅沢。",
}


# 「小節の頭に限定して切る」曲。曲名で持つ（テンプレではなく曲の性質のため）。
#
# 既定は限定しない。拍の中から強い順に選ぶだけで、swing_swing(No.32) も
# Somebody(No.37) も承認をもらえている。限定を全曲に効かせたところ Somebody が
# 「かなりずれた」になったので、必要な曲にだけ効かせる。
#
# French_Toast はバックビート（2拍4拍のスネア）が拍の中でいちばん強く、
# そのまま選ぶと小節の頭より後ろで切れて「音楽が早い・画像が遅い」になる。
BAR_LOCK = {"French_Toast"}


def bar_lock(name):
    """その音源を小節の頭だけで切るか。ファイル名でも曲名でも判定できる。"""
    k = _key(name)
    return any(_key(n) == k for n in BAR_LOCK)
