...
POSTER_SEC = {
    ...
    # 伝票は紙が伸びて品名を打ち終わるまで待つ（既定5.5秒だと2行目を印字している途中の絵になる）
    "yoshokuconto": 10.5,
    # 扉のテンプレはOPが5.0秒（表紙に今日の4品を並べ、書き終わるまで待たせる）。
    # 本編1カット目の後半＝扉が開ききり、料理名も出そろっている位置を抜く。
    "yoshokupassio": 7.3,
    # ラベルも同じくOPが5.0秒。既定5.5秒だと本編15フレーム目＝料理名が出かけの
    # 半透明状態になるので、ラベルが組み上がった本編50フレーム目を抜く。
    "yoshokuetichetta": 6.7,
    # 日除けが降りきった後、かつ皿のクロスディゾルブに当たらない位置（本編50フレーム目）。
    # 6.0秒＝本編90フレーム目はちょうど1皿目→2皿目の重なりの最中で、
    # 2皿が透けて重なった絵がポスターになっていた。
    "yoshokutenda": 4.7,
    # ベタ焼きは「シートが見えている → 1コマが引き伸ばされる → シートへ戻る」。
    # 引き伸ばしきって料理名も出ている位置（本編62〜70フレーム）を抜く。
    "yoshokuprovino": 5.4,
    # 拍で刻むテンプレは25フレームごとに料理名を出し直す。既定5.5秒＝本編75フレーム目は
    # ちょうど拍の頭（＝名前がまだ出ていない瞬間）に当たるので、拍の中ほどへずらす。
    "yoshokubattito": 6.0,
    # 石壁は3品×130フレーム。書き終わった後の“止まっている”ところを抜く
    # （本編90フレーム目＝手書きが終わって間が空いている位置）。
    "yoshokumuro": 6.0,
}
...
def main():
    ...
    HOOKS = {
        "italian": [
            "今夜は、肉。", "この一皿に乾杯を。", "肉と、赤と、いい夜と。", "旨いを、遠慮なく。",
            "腹ペコ、集合。", "日常に、ひと皿の贅沢。", "〆まで、旨い。", "肉バルの、実力。",
            "いい夜の、はじまり。", "〜コスパ良く日常に贅沢を〜",
        ],
        "french": [
            "今宵は、贅沢に。", "この一皿に金の一杯を。", "夜と、金と、フレンチと。", "本物を、気軽に。",
