...
FEED_COMPS = [
    ...
    # YoshokuFeed2.tsx の追加10種（丸い値札・黒板・半券・アーチの窓ほか）は
    # 見てもらった結果ぜんぶ不採用。「既存の6個でいい」とのことなのでここから外す。
    # コンポーネント自体は YoshokuFeed2.tsx に残してある（不採用の動画テンプレと同じ扱い）。
    # ここは毎回リスト全件を焼くので、残すと1枚あたり数十秒ぶん無駄になる。
]
...
def main():
    ...
    # H系（唯一の利用先）を廃止したので背景除去はオフ。必要になったら "1" に戻す。
