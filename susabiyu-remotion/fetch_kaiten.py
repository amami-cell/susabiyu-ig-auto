...
# 寿司系フォルダだけを対象にする（回転レーンは寿司限定）
cats = {}
for f in list_children(FOOD_FOLDER):
    if f["mimeType"] == "application/vnd.google-apps.folder" and captions.is_sushi_cat(f["name"]):
...
usage.record(creds_path, picked, "kaiten")
...
for idx, f in enumerate(picked):
    ...
    items.append({"src": "kaiten/" + local, "caption": caption})
...
if os.path.isdir(NORMAL_DIR):
    ...
    if tracks:
        ...
        music = "music/normal/" + usage.pick_bgm(tracks, creds_path)
usage.record_bgm(creds_path, music, "kaiten")
print("MUSIC:", music)
...
lines = ["export const kaitenPhotos = ["]
for it in items:
    lines.append('  { src: "%s", caption: "%s" },' % (esc(it["src"]), esc(it["caption"])))
lines.append("];")
lines.append('export const kaitenMusic = "%s";' % esc(music))
open(os.path.join("src", "kaitenData.ts"), "w", encoding="utf-8").write("\n".join(lines) + "\n")
print("src/kaitenData.ts 書き出し完了。", len(items), "枚 / music:", music)
