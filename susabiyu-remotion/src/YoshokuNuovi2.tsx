...
const FuocoBody: React.FC<Required<P>> = ({ storeName, handle, theme }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: T.base }}>
      {/* 引き（scale<1）で出る周囲は「黒い縁」ではなく同じ料理のボケ拡大で埋める＝額縁が目立たない */}
      <AbsoluteFill><Photo src={items[i].src} lf={local} seg={seg} from={1.34} to={1.26} bri={0.4} blur={34} /></AbsoluteFill>
      {/* 主役：引いた皿（合焦アニメ） */}
      ...
      {/* 四隅をほんの少しだけ落として奥行きを出す（縁は作らない・弱め） */}
      <AbsoluteFill style={{ boxShadow: "inset 0 0 160px 10px rgba(10,8,5,0.5)" }} />
      {/* ファインダーの枠（合焦すると消える） */}
