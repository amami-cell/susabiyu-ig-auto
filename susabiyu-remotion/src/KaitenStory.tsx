...
export const KaitenStory: React.FC<{ storeName?: string; handle?: string }> = ({ storeName = "すさび湯 河原町三条店", handle = "@susabiyu_sanjyo" }) => {
  ...
  const heroScale = interpolate(f, [0, DUR], [1.05, 1.13], clamp);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: BG }}>
      ...
      {/* 上：板前さんの寿司盛り合わせ写真 */}
      <div style={{ position: "absolute", top: 0, left: 0, width: 1080, height: HERO_H, overflow: "hidden" }}>
        <Img src={staticFile("itamae.jpg")} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 42%", transform: "scale(" + heroScale + ")" }} />
        <AbsoluteFill style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.34) 0%, rgba(0,0,0,0) 26%, rgba(0,0,0,0) 60%, rgba(20,12,6,0.92) 100%)" }} />
        {/* 見出し */}
