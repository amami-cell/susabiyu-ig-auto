...
export const CapOpen: React.FC<{ storeName?: string; handle?: string }> = ({ handle = "@susabiyu_sanjyo" }) => {
  ...
  const scroll = (f * 2.2) % COL_H;   // 右列は等速で無限ループ
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#f4efe8", opacity: Math.min(inO, outO) }}>
      ...
      {/* 左：ヒーロー写真2枚（上大・下小） */}
      <div style={{ position: "absolute", left: 26, top: 26, width: 610, height: 1180, borderRadius: 22, overflow: "hidden" }}>
        <Img src={staticFile(HERO[0].src)} style={{ width: "100%", height: "100%", objectFit: "cover", transform: "scale(" + heroZ + ")" }} />
      </div>
      <div style={{ position: "absolute", left: 26, top: 1232, width: 610, height: 662, borderRadius: 22, overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", left: 662, top: 0, width: 392, height: 1920, overflow: "hidden" }}>
        <div style={{ transform: "translateY(" + -scroll + "px)" }}>
          {[...FEED, ...FEED].map((p, i) => (
            <div key={i} style={{ width: 392, height: CELL - 14, margin: "0 0 14px", borderRadius: 18, overflow: "hidden" }}>
      ...
      {/* 黒い丸枠の営業テロップ */}
      <div style={{ position: "absolute", left: 60, top: 830, transform: "scale(" + blobS + ")", transformOrigin: "left center", opacity: blobO, background: "rgba(12,12,12,0.92)", borderRadius: 34, padding: "44px 54px", textAlign: "center" }}>
      ...
      <div style={{ position: "absolute", top: 40, left: 60, color: "#1c1c1c", fontFamily: serif, fontWeight: 700, fontSize: 30, letterSpacing: 2, background: "rgba(255,255,255,0.85)", borderRadius: 12, padding: "8px 18px" }}>{handle}</div>
    </AbsoluteFill>
  );
};
