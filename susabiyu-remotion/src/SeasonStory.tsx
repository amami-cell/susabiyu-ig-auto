...
export const SeasonStory: React.FC<{ storeName?: string; handle?: string }> = ({ storeName = "すさび湯 河原町三条店", handle = "@susabiyu_sanjyo" }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#0e0b08" }}>
      ...
      {/* 料理（切らずに全体表示・通常→アップ。背景はぼかしで埋める） */}
      <AbsoluteFill style={{ backgroundColor: "#0e0b08" }}>
        <Img src={staticFile(hero.src)} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "blur(26px) brightness(0.55)", transform: "scale(" + bgScale + ")" }} />
