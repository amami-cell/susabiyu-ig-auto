...
def main():
    ...
    if not (sheet_id and vapid_raw):
        print("[PUSH] SHEET_ID / VAPID_PRIVATE_KEY 未設定のため送信できません"); return 0
    ...
    for it in items:
        payload = json.dumps({
            "title": it.get("title") or "🆕 新規応募",
            "body": it.get("body") or "",
            "url": exec_url,
            "focus": "",
            "category": it.get("category") or "recruit",
            "tag": "recruit",
        })
        sent = gone = 0
        for r in subs:
            if len(r) < 2 or not str(r[1]).strip():
                continue
            if not _category_on(r[3] if len(r) > 3 else "", it.get("category") or "recruit"):
                continue
            try:
                sub = json.loads(r[1])
            except Exception:
        ...
        total_sent += sent
        print("[PUSH] '%s' → 送信 %d件 / 期限切れ %d件" % ((it.get("title") or "")[:24], sent, gone))
    print("[PUSH] 合計送信 %d件" % total_sent)
    return 0
