...
## “現代風”に見せるレシピ（iPhone/PCで遊べるまま）

写実は無理でも、「今のゲームっぽい・洗練されている」は**アートディレクション**で出せます。
参考にする方向は、スマホでも動くのに現代的に見える**スタイライズド3Dインディー**：
*A Short Hike / Alba / Lil Gator Game / Bugsnax / Untitled Goose Game* など。
＝あえて低ポリ＋良い光＋色設計。フォトリアルの逆の作法で“今っぽく”する。

効くレバー（すべて gl_compatibility＝全機種で動く。実装済み◎／今後の候補□）：
- ◎ **カラーグレーディング**（コントラスト/彩度）＝締まった発色
- ◎ **三点ライティング**（暖色キー＋青いフィル＋影の締まり）
- ◎ **ビネット（周辺減光）**＝映画的なフレーミング（`main.gd _build_vignette`）
- ◎ **地面のノーマルマップ＋PBR寄りの質感**
- ◎ **フィルミック・トーンマップ＋ブルーム＋霧**（奥行き）
- □ **統一パレット**：色数を絞り、彩度と明度の設計を1枚の“ムードボード”で固定
- □ **形の統一**：丸く・厚みのあるシルエット（すでにその方向）＋輪郭線(トゥーン)は任意
- □ **カメラ**：やや低いFOV＋被写体寄り＋わずかな手ぶれで“シネマティック”に
- □ **時間帯**：朝夕のマジックアワー固定は一番“映える”（今は昼寄り）
- □ **実写PBRテクスチャ(ambientCG/CC0)** をドロップイン＝密度と説得力が一段上がる

要点：**「解像度やPBRを盛る」のではなく、「色・光・構図を設計する」**のが、
スマホ0円運用のまま“現代的”に見せる唯一の現実解。
