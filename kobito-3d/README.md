...
### 携帯2台だけで協力プレイ（中継・費用0円）★おすすめ
携帯（ブラウザ）はホストになれないので、両方の携帯が小さな**中継**へつながって
遊びます。中継は Cloudflare Worker（無料枠）で、`relay/README.md` の手順で一度
デプロイし、`autoload/net.gd` の `RELAY_BASE` にそのURL（`wss://…workers.dev`）を
入れれば有効になります。

有効になると Web版のロビーに「みんなで遊ぶ（あいことばを作る）」が出ます。
1. 片方が「みんなで遊ぶ」を押す → **あいことば**（4文字）が出る。
2. もう片方が その あいことば を入れて「あいことばで参加する」。

同じWi-Fiでも別々のモバイル回線でもOK（中継へは外向き接続なのでCGNATを越えられます）。

### 同じWi-Fi（PCをホストに・費用0円）
