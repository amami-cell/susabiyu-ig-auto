...
func _run_selftest() -> void:
	...
	# おそうじリレー（つかむ→きれいに）が「解ける→緑が戻る」経路を確認する
	var blob := _garden.get_node_or_null("ScrubBlob") if _garden != null else null
	var blob_ok := false
	if blob != null:
		var rec3 := WorldState.recovery
		blob.debug_solve()
		await get_tree().create_timer(0.3).timeout
	...
	# 中継(Cloudflare)協力プレイのピア：id割り当て・入退室通知・配送の枠組みを検証。
	var relay_ok: bool = RelayMultiplayerPeer.run_selftest()
	...
	print("[selftest] 回復度=%.2f XP=%d 経験値=%s 飛行解禁=%s 子ども=%d(最寄り%.1f) 母=%s 石版=%s 扉=%s おそうじ=%s 輪=%s なかま=%s 種役割=%s セーブ=%s ボス召喚=%s ボス浄化=%s 図鑑=%s 音バス=%s 花あと=%s みずべ=%s ボス曲線=%s 難度=%s ごほうび=%s 笛=%s 中継=%s" % [
		WorldState.recovery, xp_now, xp_gained, flight_ok, children.size(), nearest, mother_ok, puzzle_ok, switch_ok, blob_ok, ring_ok, ally_ok, ally_species_ok, save_ok, boss_ok, boss_hold_ok, dex_ok, audio_ok, bloom_ok, water_ok, balance_ok, diff_ok, celebrate_ok, whistle_ok, relay_ok])
...
func _run_shot() -> void:
	...
	# --sky を付けると 第6章「そら」の見た目（よごれ／澄んだ空）を撮る（開発確認用）
	if OS.get_cmdline_user_args().has("--sky") and _garden != null:
		_garden.set_biome("sky")
		WorldState.set_recovery(0.15)
		await get_tree().create_timer(1.2).timeout
		await RenderingServer.frame_post_draw
		get_viewport().get_texture().get_image().save_png("/tmp/shot_sky_hazy.png")
		WorldState.set_recovery(0.9)
		await get_tree().create_timer(1.2).timeout
		await RenderingServer.frame_post_draw
	...
	# --resultshot：クリア結果カード（共有向け）を出しきった状態で撮る（開発確認用）。
	...
	if OS.get_cmdline_user_args().has("--webbugs"):
		var specs := [
			...
			"res://data/hachi.tres", "res://data/amenbo.tres", "res://data/gengoro.tres", "res://data/hotaru.tres", "res://data/dango.tres", "res://data/kumo.tres", "res://data/wata.tres",
		]
		var root := Node3D.new()
		add_child(root)
		root.global_position = Vector3(0.0, 0.0, -40.0)
	...
	if OS.get_cmdline_user_args().has("--bossshot"):
		var bspecs := [
			{"path": "res://data/queen_ant.tres", "x": -7.5},
			{"path": "res://data/tagame.tres", "x": -4.5},
			{"path": "res://data/moth.tres", "x": -1.5},
			{"path": "res://data/dustlord.tres", "x": 1.5},
			{"path": "res://data/ageha.tres", "x": 4.5},
			{"path": "res://data/sludge_lord.tres", "x": 7.5},
		]
